                    ...
                    <div className="p-6 bg-gray-50 border-t border-gray-200">
                      ...
                      <div className="mb-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
                          {[
                            { passed: !result.hasHorizontalScroll, label: '无横向滚动', desc: '页面不应超出视口宽度' },
                            { passed: result.hasViewportMeta, label: 'Viewport Meta', desc: '必须包含正确的 meta 标签' },
                            { passed: result.fontSizeReadable, label: '字体可读性', desc: '字体大小应 ≥ 12px' },
                            { passed: result.touchTargetsAdequate, label: '触摸目标', desc: '可点击元素 ≥ 44x44px' },
                            { passed: result.imagesResponsive, label: '图片响应式', desc: '图片应自适应容器' }
                          ].map((check, idx) => (
                            <div
                              key={idx}
                            ...
                            >
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className={`text-sm font-medium ${check.passed ? 'text-green-900' : 'text-red-900'}`}>
                                  {check.label}
                                </span>
                              </div>
